...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log("Invoking get-evolution-key function for status check...");
    ...
    console.log("Successfully retrieved Evolution API key for status check.");

    // Construct the Evolution API URL for status check
    const apiUrl = `${evolutionServerUrl}/instance/connectionState/${instanceId}`;
    console.log(`Supabase Function: Checking status via Evolution API: ${apiUrl}`);
    ...
    if (!evoResponse.ok) {
      ...
      console.error(`Evolution API status check error (${evoResponse.status}): ${errorText}`);
      // Return specific error structure if needed, otherwise throw
      // For simplicity, returning the status and text might be useful for the client
       return new Response(JSON.stringify({ error: `Evolution API error: ${evoResponse.statusText}`, status: evoResponse.status, details: errorText }), {
         headers: { ...corsHeaders, "Content-Type": "application/json" },
         status: evoResponse.status, // Propagate status code
       });
    }

    // Parse the JSON response from Evolution API
    const result = await evoResponse.json();
    console.log(`Supabase Function: Status check response from Evolution API for ${instanceId}:`, result);
    ...
    console.error("Error in check-whatsapp-status function:", error);
